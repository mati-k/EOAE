...
namespace EOAE_Code.Magic.Spells.BombardTargeting
{
    public abstract class BombardTargetingBase
    {
        ...
        public abstract MatrixFrame GetBestFrame(Agent caster, BombardSpell spell);
        ...
        protected static List<Agent> GetAgentsWithinVision(Agent caster, BombardSpell spell)
        {
            ...
            return humanAgents.Where(agent => agent.Position.DistanceSquared(caster.Position) <= spellDistanceSquared)
                .Where(agent => Math.Abs(caster.LookDirection.AsVec2.AngleBetween((agent.Position - caster.Position).AsVec2)) <= HALF_VISION_ANGLE)
                .ToList();
        }

        public virtual bool QuickValidate(Agent caster, BombardSpell spell)
        {
